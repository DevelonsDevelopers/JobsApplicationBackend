...
module.exports = class Offers {
    ...
    static fetch(params){
        return db.query('SELECT offers.*, jobs.title, jobs.company, jobs.role, jobs.salary, jobs.description, jobs.link, jobs.type, jobs.workdays, jobs.worktime, jobs.address, jobs.experience, jobs.qualification, jobs.skills, seekers.name as seeker_name, seekers.username, seekers.email, seekers.code, seekers.password, seekers.phone, seekers.address, seekers.dob, seekers.gender, seekers.type, companies.name as company_name FROM offers INNER JOIN jobs ON jobs.id = offers.job INNER JOIN seekers ON seekers.id = offers.user INNER JOIN companies ON jobs.company = companies.id WHERE offers.id = ?', [params.id])
    }

    static fetchByCompany(params){
        return db.query('SELECT offers.*, jobs.title, jobs.company, jobs.role, jobs.salary, jobs.description, jobs.link, jobs.type, jobs.workdays, jobs.worktime, jobs.address, jobs.experience, jobs.qualification, jobs.skills, seekers.name as seeker_name, seekers.username, seekers.email, seekers.code, seekers.password, seekers.phone, seekers.address, seekers.dob, seekers.gender, seekers.type, companies.name as company_name FROM offers INNER JOIN jobs ON jobs.id = offers.job INNER JOIN seekers ON seekers.id = offers.user INNER JOIN companies ON jobs.company = companies.id WHERE jobs.company = ?', [params.company])
    }
